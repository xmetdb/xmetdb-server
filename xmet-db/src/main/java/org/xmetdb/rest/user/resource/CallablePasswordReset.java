...
import org.xmetdb.rest.user.ForgottenPasswordReset;
import org.xmetdb.rest.user.ResetCredentials;
import org.xmetdb.rest.user.XMETCallableUserCreator;
import org.xmetdb.xmet.client.Resources;

public class CallablePasswordReset extends XMETCallableUserCreator implements IDBConfig {
	...
	protected String getURI(DBUser target, Method method) throws Exception {
		if ((count!=null) && Integer.parseInt(count.toString())==0) {
			//update failed
			return String.format("%s%s%s", baseReference, org.xmetdb.xmet.client.Resources.forgotten, Resources.failed);
		}
		if (Method.POST.equals(method) && registration!=null && target != null && target.getEmail()!=null) {
			...
			notification.sendNotification(target.getEmail(), 
					...
					String.format(emailContent,
							...
							getSystemName(),
							baseReference,
							org.xmetdb.xmet.client.Resources.forgotten,
							Resources.confirm,
					...
					"text/plain");
			return String.format("%s%s%s", baseReference, org.xmetdb.xmet.client.Resources.forgotten, Resources.notify);
