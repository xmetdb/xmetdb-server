...
public class ReadRegisteredUser   extends ReadUser<UserRegistration> implements IDBConfig {
	...
	protected static String sql = 
		"SELECT user.iduser,username,user.title,firstname,lastname,institute,weblog,homepage,email,keywords,reviewer " +
		"FROM %s.user_registration r, user x "+ 
		"where r.user_name=x.username and r.status='confirmed' and user_name=? and email=?";
